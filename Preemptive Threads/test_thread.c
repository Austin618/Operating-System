...
test_wait_kill_thread(Tid parent)
{
	...
	assert(ret == THREAD_INVALID || exitcode == THREAD_KILLED);
...
test_wait_kill(void)
{
	...
	// unintr_printf("wait_killed test done\n");
...
test_wait_parent(void)
{
	...
	for (i = 0; i < NTHREADS; i++) {
		wait[i] = thread_create((void (*)(void *))test_wait_parent_thread, (void *)(long)thread_id());
		assert(thread_ret_ok(wait[i]));
	}
