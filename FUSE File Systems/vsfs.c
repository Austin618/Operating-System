...
static void *find_address(vsfs_inode *inode, uint32_t offset) {
	vsfs_blk_t *blocks;
	uint32_t block_number = offset / VSFS_BLOCK_SIZE;
	if (VSFS_NUM_DIRECT > block_number) {
		blocks = inode -> i_direct;
	} else {
		blocks = get_fs() -> image + VSFS_BLOCK_SIZE * (inode -> i_indirect);
		block_number -= VSFS_NUM_DIRECT;
	}

	return get_fs()->image + VSFS_BLOCK_SIZE * blocks[block_number] + (offset % VSFS_BLOCK_SIZE);
}

static vsfs_dentry *find_directory_entry(uint32_t position) {
	return (vsfs_dentry *)find_address(&get_fs()->itable[0], sizeof(vsfs_dentry) * position);
}
...
static int path_lookup(const char *path,  vsfs_ino_t *ino) {
	...
	int flag = 1;
	vsfs_dentry *directory_entry;
	for (uint32_t i = 0; i < (&get_fs()->itable[0])->i_size / sizeof(vsfs_dentry); i++) {
		directory_entry = find_directory_entry(i);
		if (directory_entry->ino != VSFS_INO_MAX && strcmp((const char *)(path + 1), directory_entry->name) == 0) {
			flag = 0;
			break;
		}
	}
...
static int vsfs_getattr(const char *path, struct stat *st)
{
	...
	// define indeo number type
	vsfs_ino_t inode;
	// look up the inode for given path
	int flag = path_lookup(path, &inode);
	if (flag != 0) {
		return flag;
	}
	// inode exists => fill in the required fields based on the information stored in the node
	st-> st_mode = fs -> itable[inode].i_mode;
	st -> st_nlink = fs -> itable[inode].i_nlink;
	st -> st_size = fs -> itable[inode].i_size;
	st -> st_blocks = fs -> itable[inode].i_blocks;
	st -> st_mtim = fs -> itable[inode].i_mtime;
	return 0;
}
                        ...
                        off_t offset, struct fuse_file_info *fi)
{
	...
	if (strcmp(path, "/") == 0) {
		for (uint32_t i = 0; i < (&get_fs() -> itable[0]) -> i_size / sizeof(vsfs_dentry); i++) {
			vsfs_dentry *directory_entry = find_directory_entry(i);
			if (directory_entry -> ino != VSFS_INO_MAX) {
				int temp = filler(buf, directory_entry -> name, NULL, 0);
				if (temp == 1) {
					return temp;
				}
			}
...
static int vsfs_unlink(const char *path)
{
	...
	path_lookup(path, &inode_index);

	vsfs_dentry *directory_entry;
	for (uint32_t i = 0; i < (&get_fs() -> itable[0]) -> i_size / sizeof(vsfs_dentry); i++) {
		directory_entry = find_directory_entry(i);
		if (directory_entry -> ino != VSFS_INO_MAX && strcmp(directory_entry -> name, (const char *)(path + 1)) == 0) {
			memset(directory_entry, 0, sizeof(vsfs_dentry));
			directory_entry -> ino = VSFS_INO_MAX;
			clock_gettime(CLOCK_REALTIME, &((&fs -> itable[0]) -> i_mtime));
		}
	}

	bitmap_free(fs -> ibmap, fs -> sb -> num_inodes, inode_index);
	fs -> sb -> free_inodes += 1;
	return 0;
                     ...
                     struct fuse_file_info *fi)
{
	...
	vsfs_inode *inode;
	path_lookup(path, &inode_index);
	inode = &(fs -> itable[inode_index]);

	if ((long int)inode -> i_size < offset) {
		return 0;
	}
	...
	memcpy(buf, (char *)find_address(inode, offset), size);
	return (int)size;
                      ...
                      off_t offset, struct fuse_file_info *fi)
{
	...
	memcpy((char *)find_address(inode, offset), buf, size);
