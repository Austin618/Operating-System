...
static bool mkfs(void *image, size_t size, mkfs_opts *opts)
{
	...
	uint32_t total_blocks = div_round_up(opts -> n_inodes, inodes_per_block);
	for (uint32_t i = 0; i < total_blocks; i++) {
		bitmap_set(dbmap, nblks, VSFS_DMAP_BLKNUM + 1 + i, true);
	}
	...
	// vsfs_dentry *root_entries; // ptr to root dir data block in mmap'd image
	root_entries = (vsfs_dentry *)((char *)image + VSFS_BLOCK_SIZE * (root_ino -> i_direct[0]));
	...
	for (uint32_t j = 2; j < div_round_up(VSFS_BLOCK_SIZE, sizeof(vsfs_dentry)); j++) {
	...
	sb -> num_blocks = nblks;
	sb -> free_blocks = sb -> num_blocks - VSFS_DMAP_BLKNUM - total_blocks - 2;
	sb -> free_inodes = opts -> n_inodes - 1;
	sb -> data_region = VSFS_ITBL_BLKNUM + total_blocks;
